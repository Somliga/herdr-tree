...
func (s *Store) SummariesFor(sessionID string) []Summary {
	...
	// Stable, with a tie-break on the key: the source is a map, whose
	// iteration order Go randomises, and CreatedAt ties are ordinary (a caller
	// may leave it zero). Without this the picker in Task 9 reshuffles between
	// opens, which no test would catch and which makes a list feel broken.
...
In `view.go`, have `Run` compute the trunk once and call `SetTrunk` before the
first render. Do NOT touch styling here — `styleFor` does not exist until
Task 7, which consumes `Row.OnTrunk` when it arrives:
...
**Also extend `fakeAdapter`** in `internal/tui/view_test.go`, in this same
commit. It is the interface's only other implementation, and adding a method
without it breaks the build of a package two tasks before the task that would
have fixed it:
...
**Also extend `fakeAdapter`** in `internal/tui/view_test.go`, in this same
commit, for the same reason as Task 4:
...
func TestFoldBackAtAnEarlierTurnSeedsAGraft(t *testing.T) {
	...
	msg := foldBackCmd(fa, st, n, "/repo", sum, "", nil)().(actionDoneMsg)
	if fa.seededWith == "" {
...
func TestFoldBackAtTheLiveTipSendsAMessage(t *testing.T) {
	...
	foldBackCmd(fa, st, n, "/repo", sum, "tree-agent", send)()
	if fa.seededWith != "" {
		t.Fatal("the live tip must not be grafted; a 6.6MB copy to deliver one message")
	}
	if gotAgent != "tree-agent" || !strings.Contains(gotText, "folded") {
		t.Fatalf("expected a send, got agent=%q text=%q", gotAgent, gotText)
	}
}
```

Extend `fakeAdapter` with `summary string` and `seededWith string`, and make
its `Summarise` and `BranchSeeded` record their arguments rather than returning
the fixed stubs Tasks 4 and 5 gave them. Agent delivery is captured by the
injected `SendFunc`, not by the adapter.

`uiModel` gains a `send SendFunc` field, set by `Run`; `cmd/herdr-tree` passes
`herdr.AgentPrompt`. `internal/tui` must still not import `internal/herdr` —
verify with `go list -deps ./internal/tui`.
...
**Before building the picker, fix `SummariesFor`'s ordering** in
`internal/store/store.go` — it uses `sort.Slice` over a map, so summaries with
equal `CreatedAt` (including two zero values) reshuffle between opens. Replace
it with the stable, tie-broken form now in Task 1's text, and add a test that
builds three summaries with identical timestamps and asserts the order is the
same across twenty calls. `esc` cancels either.
